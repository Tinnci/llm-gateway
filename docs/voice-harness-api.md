...
- `status`, `route`, and `provider`: exact-match filters;
...
contains identifiers, bounded reply text, route, latency, tools, and error
types. It does not contain the event stream or a decompressed raw payload.
